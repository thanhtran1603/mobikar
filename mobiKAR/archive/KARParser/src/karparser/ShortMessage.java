...
public class ShortMessage extends MidiMessage {
     ...
     * @throws  <code>Exception</code> if <code>status</code> does not
     * specify a valid MIDI status byte for a message that requires no data bytes.
     * @see #setMessage(int, int, int)
     * @see #setMessage(int, int, int, int)
     */
    public void setMessage(int status) throws Exception {
	// check for valid values
	int dataLength = getDataLength(status); // can throw Exception
	if (dataLength != 0) {
	    throw new Exception("Status byte; " + status + " requires " + dataLength + " data bytes");
	}
     ...
     * @throws	<code>Exception</code> if the
     * the status byte, or all data bytes belonging to the message, do
     * not specify a valid MIDI message.
     * @see #setMessage(int, int, int, int)
     * @see #setMessage(int)
     */
    public void setMessage(int status, int data1, int data2) throws Exception {
	// check for valid values
	int dataLength = getDataLength(status); // can throw Exception
	if (dataLength > 0) {
	    if (data1 < 0 || data1 > 127) {
		throw new Exception("data1 out of range: " + data1);
	    }
	    if (dataLength > 1) {
		if (data2 < 0 || data2 > 127) {
		    throw new Exception("data2 out of range: " + data2);
		}
	    }
	}
     ...
     * @see #getData2
     */
    public void setMessage(int command, int channel, int data1, int data2) throws Exception {
	// check for valid values
	if (command >= 0xF0 || command < 0x80) {
	    throw new Exception("command out of range: 0x" + Integer.toHexString(command));
	}
	if ((channel & 0xFFFFFFF0) != 0) { // <=> (channel<0 || channel>15)
	    throw new Exception("channel out of range: " + channel);
	}
     ...
     * short message
     */
    protected final int getDataLength(int status) throws Exception {
