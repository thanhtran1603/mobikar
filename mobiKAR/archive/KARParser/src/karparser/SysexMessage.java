...
public class SysexMessage extends MidiMessage {
     ...
     */
    public void setMessage(byte[] data, int length) throws Exception {
	int status = (data[0] & 0xFF);
	if ((status != 0xF0) && (status != 0xF7)) {
	    throw new Exception("Invalid status byte for sysex message: 0x" + Integer.toHexString(status));
	}
     ...
     * the array
     */
    public void setMessage(int status, byte[] data, int length) throws Exception {
	if ( (status != 0xF0) && (status != 0xF7) ) {
	    throw new Exception("Invalid status byte for sysex message: 0x" + Integer.toHexString(status));
	}
