...
public class MetaMessage extends MidiMessage {
     ...
     */
    public void setMessage(int type, byte[] data, int length) throws Exception {

	if (type >= 128 || type < 0) {
	    throw new Exception("Invalid meta event with type " + type);
	}
	if ((length > 0 && length > data.length) || length < 0) {
	    throw new Exception("length out of bounds: "+length);
	}
